...
(function () {
  ...
  var css = [
    ...
    /* push page content clear of the fixed topbar + sidebar (desktop only reserves */
    /* space for the sidebar; mobile/tablet never does — sidebar overlays there). */
    ...
    /* Desktop: header logo hidden while sidebar is open (stacked sidebar logo shows */
    /* instead), shown again once collapsed. Mobile/tablet: always shown. Hidden */
    /* entirely below 450px regardless of state (see override after the 1023px rule). */
    ...
    "@media(max-width:1023px){#ct-logo{display:block !important;}}",                       /* mobile/tablet: always shown */
    "@media(max-width:449px){#ct-logo{display:none !important;}}",                         /* below 450px: hidden entirely */

    /* ── Sidebar — sits below the topbar, never overlaps it (lower z-index) ── */
    ...
    /* ── Mobile: off-canvas overlay (content only, never the header) ── */
  ...
  function build() {
    ...
    topbar.innerHTML =
      '<div class="ct-left">' +
        ...
        '</button>' +
        '<picture>' +
          '<source media="(max-width:650px)" srcset="/logo_mobile.png">' +
